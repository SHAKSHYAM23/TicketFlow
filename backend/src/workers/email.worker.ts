...
const sendConfirmationEmail = async (payload: NotificationPayload): Promise<void> => {
  ...
  if (!booking || !booking.user.email) throw new Error('Booking/User not found')
  ...
  channel.consume(QUEUES.NOTIFY, async (msg) => {
    ...
    try {
      ...
      if (retryCount >= MAX_RETRIES) {
        channel.sendToQueue('notify_failed', Buffer.from(JSON.stringify({ ...payload, finalError: (err as Error).message })), { persistent: true })
        channel.ack(msg)
        dlqCounter.inc()
      } else {
