...
const handleNotificationRequested = async (
  payload: NotificationRequestedPayload
): Promise<void> => {

  console.log(
    `notification.requested received — booking: ${payload.bookingId}`
  )
  ...
  await notificationConsumer.run({
    ...
    eachMessage: async ({ topic, message, partition }) => {
      ...
      try {
        ...
        console.error(
          `Failed to process notification.requested — ` +
          `RabbitMQ may be down. Message NOT committed, will retry.`,
          err
        )
