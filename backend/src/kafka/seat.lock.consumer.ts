...
const startRedisExpirySubscriber = async (): Promise<void> => {
  ...
  await subscriber.subscribe(
    ...
    async (expiredKey: string) => {
      ...
      if (!expiredKey.startsWith('lock:seat:')) return
...
const handleSeatLockExpired = async (
  ...
  console.log(
    ` Socket emitted GREEN for expired seat: ${seat.rowLabel}${seat.seatNumber}`
  )
}



export const startSeatLockConsumer = async (): Promise<void> => {
