...
function availability(event: EventItem) {
  ...
  if (available <= 10) {
    return {
      label: available <= 0 ? 'Sold out' : `${available} seats left`,
...
export default function EventCard({ event, onClick }: EventCardProps) {
  ...
  return (
    <button
      ...
      className="group flex w-full flex-col rounded-xl border border-border bg-card p-5 text-left transition-colors hover:border-primary/60"
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {hasAvailability && (
          <Badge className={`shrink-0 ${avail.className} hover:${avail.className}`}>
            {avail.label}
          </Badge>
        )}
      </div>
